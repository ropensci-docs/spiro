...
- Use [`spiro()`](https://docs.ropensci.org/spiro/reference/spiro.md) to
  automatically import and process raw data from cardiopulmonary
  exercise testing.
- Use
  [`spiro_summary()`](https://docs.ropensci.org/spiro/reference/spiro_summary.md)
  for a summary of cardiopulmonary parameters (e.g., relative oxygen
  uptake, respiratory quotient, heart rate, …) for each load step.
- Use
  [`spiro_max()`](https://docs.ropensci.org/spiro/reference/spiro_max.md)
  to calculate maximum parameter values (e.g., VO2max).
- Use
  [`spiro_plot()`](https://docs.ropensci.org/spiro/reference/spiro_plot.md)
  to visualize the data as a modifiable Wassermann 9-Panel Plot.

Further functionality:

- Add external heart rate data from a .tcx file.
- Automated guessing or manual setting of exercise protocols.
- Different data filtering strategies for VO_(2max) determination
  (moving time averages, moving breath averages, Butterworth filters)
